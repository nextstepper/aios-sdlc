...
def get_memory_client():
    """Return singleton mem0 client.
    Cloud mode: MEM0_API_KEY set → uses MemoryClient (no local LLM/embedder needed).
    Self-hosted: falls back to Memory.from_config() using mem0_config.yaml.
    """
    global _client
    if _client is not None:
        return _client

    cloud_key = os.getenv("MEM0_API_KEY")
    if cloud_key:
        from mem0 import MemoryClient
        _client = MemoryClient(api_key=cloud_key)
    else:
...
def mem0_search(query, limit=10):
    """Search memories — handles cloud vs self-hosted param differences."""
    m = get_memory_client()
    if is_cloud_mode():
...
def mem0_add(messages=None, content=None, metadata=None):
    """Add memory — handles cloud vs self-hosted param differences."""
    m = get_memory_client()
    ...
    return m.add(content, **kwargs)


def mem0_get_all(limit=100):
    """List all memories — handles cloud vs self-hosted param differences."""
    m = get_memory_client()
    if is_cloud_mode():
        return m.get_all(filters={"user_id": USER_ID}, limit=limit)
