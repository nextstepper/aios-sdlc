...
Scripts location: `${CLAUDE_PLUGIN_ROOT}/skills/memory/scripts/`

## Operations

### Search memories (smart — recommended)
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/memory/scripts/smart_search.py --query "topic" --limit 5
```
...
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/memory/scripts/mem0_search.py --query "topic" --limit 10
```

### Rebuild keyword index
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/memory/scripts/smart_search.py --rebuild-index
```
Repopulates the FTS5 keyword index from the history DB. Run once after installation, or periodically to fix drift.

### Add a specific fact
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/memory/scripts/mem0_add.py --content "User prefers Pinecone over Supabase for vectors"
```

### Add from conversation messages
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/memory/scripts/mem0_add.py --messages '[{"role":"user","content":"I switched to ClickUp"}]'
```

### Append to daily session log
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/memory/scripts/daily_log.py --content "Completed memory system overhaul" --type event
```

### Sync mem0 to MEMORY.md
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/memory/scripts/mem0_sync_md.py           # Regenerate MEMORY.md from mem0
python3 ${CLAUDE_PLUGIN_ROOT}/skills/memory/scripts/mem0_sync_md.py --dry-run  # Preview changes
```

### List all memories
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/memory/scripts/mem0_list.py --limit 50
```

### Delete a memory
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/memory/scripts/mem0_delete.py --memory-id "abc123"
python3 ${CLAUDE_PLUGIN_ROOT}/skills/memory/scripts/mem0_delete.py --all --confirm
```
...
- mem0 config: `${CLAUDE_PLUGIN_ROOT}/skills/memory/references/mem0_config.yaml`
